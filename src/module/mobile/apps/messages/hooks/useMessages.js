...
const useMessages = () => {
  ...
  const [pinnedChats, setPinnedChats] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : ["vedant"];
  });

  const [activeCategory, setActiveCategory] = useState("all"); // "all", "unread", "muted"
  const [activeChatId, setActiveChatId] = useState("vedant");
  ...
  const sendAttachment = useCallback(
    (type, value) => {
      ...
      // Vedant chatbot reply triggers
      if (activeChat.id === "vedant") {
        setIsTyping(true);
        setTimeout(() => {
          setIsTyping(false);
          const replyMessage = {
          ...
          setConversations((prev) =>
            prev.map((c) =>
              c.id === "vedant" ? { ...c, messages: [...c.messages, replyMessage] } : c,
            ),
          );
        }, 1200);
      }
    ...
    if (activeChat.id === "vedant") {
      ...
      setTimeout(() => {
        ...
        if (userMsg.includes("project")) {
          replyText =
            "I have built several cool projects! You can check them out in the 'Portfolio' Finder app, including 'Botlock', 'NexusAI', 'fetchit', and 'NeverSleep'.";
        } else if (
          ...
          replyText =
            "My primary tech stack includes Python, C++, JavaScript, React, Next.js, Node.js, Express, TensorFlow, Scikit-learn, PostgreSQL, and MongoDB. Open the 'Skills' Terminal app to see the complete list!";
        } else if (
          ...
          replyText =
            "You can contact me via email at vedant@example.com, or check out my socials (Github, LinkedIn) in the Safari app!";
