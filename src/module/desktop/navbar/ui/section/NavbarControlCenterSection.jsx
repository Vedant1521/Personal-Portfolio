...
const NavbarControlCenterSection = ({
  ...
  const [profile, setProfile] = useState({
    ...
    bio: "Full-Stack Developer & Mathematics and Computing Undergrad",
  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/users/${GITHUB_USERNAME}`)
      ...
      .then((data) => {
        if (data && data.login) {
          setProfile({
            name: data.name || "Vedant Gupta",
            avatar_url: data.avatar_url,
            login: data.login,
            email: data.email || `${data.login}@users.noreply.github.com`,
            bio: data.bio || "Full-Stack Developer & Mathematics and Computing Undergrad",
  ...
  const wifiNetworksList = [
    ...
    { name: "iPhone (Vedant)", locked: true, strength: "strong" },
  ...
  const bluetoothDevicesList = [
    ...
    { key: "headphones", name: "Sony WH-1000XM5", icon: "🎧" },
  ];
                      ...
                      <div className="my-3 text-[11.5px] text-white/80 leading-normal flex items-start gap-1.5">
                        ...
                        {currentUserMode === "admin" ? (
                            <span className="block font-medium">
                              Full-Stack Developer & Mathematics and Computing Undergrad | Building beautiful, premium web solutions.
                            </span>
                        ) : (
                          <span>
