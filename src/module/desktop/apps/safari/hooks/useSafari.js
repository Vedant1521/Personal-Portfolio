...
const useSafari = () => {
  ...
  const navigateTabTo = (url) => {
    ...
    if (lowerQuery.includes("library") || lowerQuery.includes("botlock")) {
      targetUrl = PROJECT_1_URL;
      isRedirected = true;
    } else if (lowerQuery.includes("packet") || lowerQuery.includes("nexus")) {
      targetUrl = PROJECT_2_URL;
      isRedirected = true;
    } else if (lowerQuery.includes("asset") || lowerQuery.includes("fetchit")) {
      targetUrl = PROJECT_3_URL;
      isRedirected = true;
    } else if (lowerQuery.includes("neversleep") || lowerQuery.includes("sleep")) {
  ...
  const getTitleForUrl = (url) => {
    ...
    if (url === PROJECT_1_URL) return "Botlock";
    if (url === PROJECT_2_URL) return "NexusAI";
    if (url === PROJECT_3_URL) return "fetchit";
    if (url === PROJECT_4_URL) return "NeverSleep";
